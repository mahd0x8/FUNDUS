"""
train_swin.py — Swin-Large Transformer for multi-label fundus disease classification.
...
Outputs      : EXPERIMENTS/SWIN_V1/
  - best_swin.pt          (best val macro-F1 checkpoint)
  - swin_evaluation.txt   (per-class + macro metrics on test set)
  - training_curves.png
...
# ============================================================
# MODEL
# Backbone : Swin-Large (swin_large_patch4_window7_224)
# Features : 1536-d global average-pooled representation
# Head     : Linear(1536→512) → GELU → Dropout → Linear(512→C)
# ============================================================
...
class SwinClassifier(nn.Module):
    def __init__(self, num_classes: int, pretrained: bool = True, dropout: float = 0.3):
        ...
        feature_dim = self.backbone.num_features   # 1536 for Swin-L
...
class SwinConfig:
    ...
    num_workers:  int   = 4
    seed:         int   = 42
    out_dir:      str   = "EXPERIMENTS/SWIN_V1/"
...
def make_loaders(df: pd.DataFrame, cfg: SwinConfig):
...
def format_table(metrics: dict, thresholds: np.ndarray) -> list[str]:
...
def print_metrics(metrics: dict, thresholds: np.ndarray):
    print("\n".join(format_table(metrics, thresholds)))
...
def main(cfg: SwinConfig):
    ...
    log(f"Parameters : {n_params:.1f}M  (Swin-Large backbone + head)")

    # Differential LR: backbone gets a much smaller rate than the head
    ...
    # ---- Training loop ----
    best_val_f1  = -1.0
    best_path    = os.path.join(cfg.out_dir, "best_swin.pt")
    train_losses = []
    val_f1s      = []

    log(f"\n{'='*60}")
    log(f"  SWIN-LARGE TRAINING  ({cfg.epochs} epochs, warmup={cfg.warmup_epochs})")
    log(f"  Backbone LR : {cfg.lr_backbone}   Head LR : {cfg.lr_head}")
    log(f"  Batch size  : {cfg.batch_size}   Dropout : {cfg.dropout}")
    log(f"  Weight decay: {cfg.weight_decay}  Grad clip: {cfg.grad_clip}")
    log(f"{'='*60}\n")
    ...
    for epoch in range(cfg.epochs):
        ...
        if val_loader is not None:
            ...
            msg = (
                f"[Swin] Epoch {epoch+1:03d}/{cfg.epochs}  "
    ...
    save_metrics(metrics, thresholds, os.path.join(cfg.out_dir, "swin_evaluation.txt"))
